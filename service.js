...
/* =========================================
   SERVICE PAGE — service.js
   ========================================= */

/* =========================================
   STEP 1 — Mark body as loaded so CSS
   reveal rules activate AFTER paint
   ========================================= */
window.addEventListener('DOMContentLoaded', () => {

  /* Small delay so first paint renders everything visible */
  setTimeout(() => {
    document.body.classList.add('sv-loaded');
    initReveal();
  }, 100);
...
function initAll() {
  ...
  if (form) {
    form.addEventListener('submit', e => {
      ...
      const fbtn  = form.querySelector('button');
  ...
  /* ── FAQ ACCORDION ── */
  document.querySelectorAll('.sv-faq-q').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      /* close all */
      document.querySelectorAll('.sv-faq-item').forEach(other => {
        other.querySelector('.sv-faq-q').setAttribute('aria-expanded', 'false');
        other.querySelector('.sv-faq-a').classList.remove('is-open');
      });
  ...
  /* ── PRICING HOVER DIM ── */
  document.querySelectorAll('.sv-plan').forEach(plan => {
    plan.addEventListener('mouseenter', () => {
      document.querySelectorAll('.sv-plan').forEach(p => {
        if (p !== plan) p.style.opacity = '0.6';
      });
    });
    plan.addEventListener('mouseleave', () => {
      document.querySelectorAll('.sv-plan').forEach(p => p.style.opacity = '1');
    });
  });
  ...
  /* ── HERO PARALLAX ── */
  const heroBg = document.querySelector('.sv-hero-bg');
  if (heroBg) {
    window.addEventListener('scroll', () => {
  ...
  const statEls = document.querySelectorAll('.sv-istat strong');
    ...
    entries.forEach(e => {
      if (e.isIntersecting) {
        statEls.forEach((el, i) => {
          if (statDefs[i]) {
            setTimeout(() => animateCount(el, statDefs[i].target, statDefs[i].suffix, 1400), i * 200);
          }
        });
  ...
  const introSec = document.getElementById('svIntro');
  if (introSec) introObs.observe(introSec);

  /* ── PROCESS STEP NUMBERS COUNT-UP ── */
  document.querySelectorAll('.sv-step-num').forEach(el => {
...
/* =========================================
   SCROLL REVEAL — runs after sv-loaded
   ========================================= */
function initReveal() {
...
function animateCount(el, target, suffix, duration) {
  ...
  function tick(now) {
    ...
    const v = 1 - Math.pow(2, -10 * p); /* easeOutExpo */
